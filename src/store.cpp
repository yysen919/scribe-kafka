...
Store::createStore(StoreQueue* storeq, const string& type,
  ...
  } else if (0 == type.compare("kafka")) {
    return shared_ptr<Store>(new KafkaStore(storeq,category, multi_category));
...
KafkaStore::KafkaStore(StoreQueue* storeq,
                        const string& category,
                        bool multi_category)
  : Store(storeq, category, "kafka", multi_category),
    rk(NULL), rkt(NULL), conf(NULL), topic_conf(NULL), 
    openStat(false), partition(RD_KAFKA_PARTITION_UA),
    maxMessagesQueueLen(5000000), maxTime(3), curMaxTime(0){

    msg_queue = boost::shared_ptr<msg_queue_t>(new msg_queue_t);
}
...
void KafkaStore::configure(pStoreConf configuration, pStoreConf parent) {
    ...
    if (!configuration->getString("kafkaConfFile", kafkaConfFile)){
        LOG_OPER("store kafka need kafkaConfFile config");
        exit(1);
    }
    if (!configuration->getString("topic", topic)) {
        LOG_OPER("store kafka need topic config");
        exit(1);
    }
    unsigned long int  tmp;
    if (configuration->getUnsigned("partition", tmp)) {
        partition = tmp;
    }
    if (configuration->getUnsigned("maxMessagesQueueLen", tmp)) {
        maxMessagesQueueLen = tmp;
    }
    if (configuration->getUnsigned("maxTime", tmp)) {
        maxTime = tmp;
    }
}
...
bool KafkaStore::open() {
    ...
    if (rk && rkt) {
        if (rd_kafka_outq_len(rk) < maxMessagesQueueLen) {
            openStat = true;
            return openStat;
        }
        else {
    ...
    kafkaConfInit(&conf, &topic_conf, 30);


    rk = rd_kafka_new(RD_KAFKA_PRODUCER, conf, errstr, sizeof(errstr));
    if (!rk) {
        LOG_OPER("new kafka error=[%s]", errstr);
        return false;
    }

    rkt = rd_kafka_topic_new(rk, topic.c_str(), topic_conf);
    if (!rkt) {
        LOG_OPER("new kafka topic  error=[%s]", errstr);
        rd_kafka_destroy(rk);
        rk = NULL;
        return false;
    } 
...
void KafkaStore::kafkaConfInit(rd_kafka_conf_t **conf, rd_kafka_topic_conf_t **topic_conf, int timeout) {
    ...
    *topic_conf = rd_kafka_topic_conf_new();
    ...
    fp = fopen(kafkaConfFile.c_str(), "r");
    ...
    while (fgets(buf, sizeof(buf)-1, fp)) {
        ...
        if (*b == '#' || !*b)
            continue;
        if (!(t = strchr(b, '=')))
            printf("%s:%i: expected name=value format\n", "", line);
        ...
        *t = '\0';
        ...
        if (!strncmp(name, "topic.", strlen("topic."))) {
            name += strlen("topic.");
            printf("%s=%s\n", name, val);
            res = rd_kafka_topic_conf_set(*topic_conf, name, val, errstr, sizeof(errstr));

        } else {
            printf("%s=%s\n", name, val);
            if (!strcmp(name, "queue.buffering.max.messages")) {
                maxMessagesQueueLen = atoi(val);
                continue;
            }
    ...
    char val[15] ={0};
    sprintf(val, "%d", maxMessagesQueueLen * 2);
    res = rd_kafka_conf_set(*conf, "queue.buffering.max.messages", val, errstr, sizeof(errstr));
    if (res != RD_KAFKA_CONF_OK) {
        LOG_OPER(" set topic queue.buffering.max.messages=[%s] error", val);
    }
    fclose(fp);
    //rd_kafka_conf_set_error_cb(*conf, NULL);
    //rd_kafka_conf_set_dr_cb(*conf, new kafkaMsgDelivered);
}
...
boost::shared_ptr<Store> KafkaStore::copy(const std::string &category) {
    ...
    store->kafkaConfFile = this->kafkaConfFile;
    return copied;
}
...
bool KafkaStore::handleMessages(boost::shared_ptr<logentry_vector_t> messages) {
    ...
    if (!isOpen()) {
        if (!open()) { 
            LOG_OPER("when handleMsg store is close and can't open store");
        return false;
        }
    }
    ...
    for (logentry_vector_t::iterator iter = messages->begin();
        ...
        if (failed_messages->size() > 0) { 
            failed_messages->insert(failed_messages->end(), iter, messages->end());
            break;
        }

        string  m= (*iter)->message;
        const char * msg = m.c_str();
        char * tmp = const_cast<char *>(msg);//少用这种转换
        uint32_t i = 0;
        int pos  =0;
        if (rd_kafka_outq_len(rk) >= maxMessagesQueueLen) {
        ...
        for (i = 0; i < m.length(); i++) {
            if (msg[i] == '\n') {
                //void * s = malloc(i - pos);
                //memcpy(s, msg + pos, i - pos);
                int ret =  rd_kafka_produce(rkt, partition, RD_KAFKA_MSG_F_COPY, tmp + pos, i - pos, NULL, 0, NULL);
                if (ret == -1) {
                    missed_msg++;
                    int error = rd_kafka_errno2err(errno);
                    //LOG_OPER("missed a messages errno==[%d] error=[%d]", errno, error);
                    if (error == ENOBUFS) { 
                        LOG_OPER("reason is ENOBUFS");
                    } else if (error == EMSGSIZE) { 
                        LOG_OPER("reason is EMSSIZE");
                    } else if (error == ESRCH) {
                        LOG_OPER("reason is ESRCH");
                    } else if (error == ENOENT) { 
                        LOG_OPER("reason is ENOENT");
                    }
                } else {
                    pos = i + 1;
                }
    ...
    if (missed_msg)
        LOG_OPER("missed [%d] messages, rd_kafka_outq_len = [%d] ", missed_msg, rd_kafka_outq_len(rk));
...
void KafkaStore::periodicCheck() {
    int event_num = rd_kafka_poll(rk, 0);
    int queue_len = rd_kafka_outq_len(rk);
    LOG_OPER("[debug] kafka producer message quene len = [%d], event_num = [%d]n", queue_len, event_num);
    if (queue_len >= maxMessagesQueueLen) {
        curMaxTime++;
    ...
    msg_queue_t::iterator iter;
    for (iter = msg_queue->begin(); iter != msg_queue->end(); iter++) {
        rd_kafka_produce(rkt, partition, RD_KAFKA_MSG_F_COPY, const_cast<char *>((*iter)->c_str()), (*iter)->length(), NULL, 0, NULL);
    }
...
/*
void KafkaStore::kafkaMsgDelivered(rd_kafka_t *rk,void *payload, size_t len,int error_code,void *opaque, void *msg_opaque){

    if (error_code) {
        shared_string_t s(new std::string(static_cast<const char *>(payload), len));
        msg_queue->push_back(s);
        LOG_OPER("send msg =[%s] failed [%d] ", s->c_str(), error_code);
    }
}
*/
std::string KafkaStore::getStatus() {
