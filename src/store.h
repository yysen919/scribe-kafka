...
/*
   A class  that serves as kafka stores
*/
class KafkaStore : public Store {
 public:
  KafkaStore(StoreQueue* storeq,
 ...
 protected:
  ...
  // Configuration
  std::string kafkaConfFile;
  std::string topic;
  int partition;
  int maxMessagesQueueLen;
  int maxTime;
  ...
  typedef boost::shared_ptr<std::string> shared_string_t; 
  typedef std::vector<shared_string_t> msg_queue_t;
  boost::shared_ptr<msg_queue_t> msg_queue;
...
bool openStat;
 ...
 private:
  void kafkaConfInit(rd_kafka_conf_t **conf, rd_kafka_topic_conf_t **topic_conf, int timeout);
  void kafkaMsgDelivered(rd_kafka_t *rk,void *payload, size_t len,int error_code,void *opaque, void *msg_opaque);
  /*
  class kafkaMsgDelivered {
      public:
      void operator()(rd_kafka_conf_t **conf, rd_kafka_topic_conf_t **topic_conf, int timeout) {
          if (error_code) {
